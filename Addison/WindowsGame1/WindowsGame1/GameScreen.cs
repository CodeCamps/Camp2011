...
namespace WindowsGame1
{
    class GameScreen : Screen
    {
        ...
        public override void LoadContent(Rectangle screenBounds)
        {
            ...
            texSprites = TheGame.Content.Load<Texture2D>("addisonsprites");
            texClouds = TheGame.Content.Load<Texture2D>("clouds");

            maxX = 2 * bounds.Width / 3;
            restingX = (maxX - 10) / 5 + 10;

            Rectangle rect = new Rectangle(10, 10, 80, 96);
            ...
            thePlayer1.SourceRect = new Rectangle(0, 409, 80, 96);
        ...
        public void AddSeed(Player player)
        {
            ...
            seed.DestinationRect = new Rectangle(573, 285, 37, 17);
            seed.SourceRect = new Rectangle(573, 285, 28, 10);
        ...
        public void AddEgg(Player player, Color tint)
        {
            ...
            egg.Tint = tint;
            egg.SourceRect = 
            egg.DestinationRect = new Rectangle(645, 300, 48, 62);
        ...
        public void AddTree()
        {
            ...
            var branch = new TreeBranch(
                ...
                new Rectangle(bounds.Width + 2, bounds.Height - 233 - 6, 48, 127),
                -2.1f);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            //{
            //    if (birds[playerIndex].birdDied)
            //    {
            //        birds.RemoveAt(playerIndex);
            //    }
            //}
            //index = 0;
            //while (index < birds.Count)
            //{
            //    if (birds[index].birdDied)
            //    {
            ...
            //{
            //    GamePad.SetVibration(thePlayer1.playerIndex, 0.0f, 1.0f);
            //    GamePad.SetVibration(thePlayer2.playerIndex, 0.0f, 1.0f);
            //}
            //else
            //{
            ...
            foreach (var player in birds.Values)
            {
                if (!player.birdDied)
                {
                    foreach (var pig in pigs)
                    {
                        if (pig.CanDie)
                        {
                            if (player.DestinationRect.Intersects(pig.DestinationRect))
                            {
                                ...
                                // TODO: replace moving to 10, 10 to making transparent for a bit.
                                player.DestinationRect.X =
                                    player.DestinationRect.Y = 10;
                                player.PlayerHealth--;
                                //birdDied = true;
                                player.birdDamage = true;
                            }
                        }
                    }
                    ...
                    foreach (var pig in pigs)
                    {
                        ...
                        {
                            if (player.DestinationRect.Intersects(pig.DestinationRect) &&
                                player.DestinationRect.Y - pig.DestinationRect.Y >= 0)
                            {
                                player.DestinationRect.Y -= (pig.DestinationRect.Y / 2);
                            //    player.DestinationRect.X =
                            //        player.DestinationRect.Y = 10;
                                player.birdDamage = true;
                                player.PlayerHealth--;
                    ...
                    if (player.birdDamage) { effectSquawk.Play(); }
                }
            }
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            ...
            Rectangle heartLoc = new Rectangle(10, 40, 15, 15);
            ...
            foreach (var player in birds.Values)
            {
                    ...
                    heartLoc.X = rectBird.X;

                while (player.PlayerHealth > 0 && heartIndex <= player.PlayerHealth)
                {
                    spriteBatch.Draw(
                        texSprites,
                        heartLoc,
                        new Rectangle(646, 107, 50, 47),
                        Color.White);
